...
class Player(pygame.sprite.Sprite):
	...
	def actualiza_texto(self):
		if texto_dictado==texto_objetivo and self.rect.bottom >= 250:
			self.gravity = -20
			# self.jump_sound.play()
...
class Obstacle(pygame.sprite.Sprite):
	def __init__(self,type):
		super().__init__()
		objetivo_message = test_font.render(f'{texto_objetivo}',False,(111,196,169))
		...
		if type == 'nube':
			...
			nube_1 = pygame.image.load('graphics/nube/nube1.png').convert_alpha()
			nube_2 = pygame.image.load('graphics/nube/nube1.png').convert_alpha()
			self.frames = [nube_1,nube_1]
			y_pos  = 100
...
text_surface = test_font.render(user_text, False, (255,255,255))
...
dictado_timer = pygame.USEREVENT + 1
dictado_reset = pygame.USEREVENT + 1
pygame.time.set_timer(obstacle_timer,1500)
pygame.time.set_timer(dictado_timer,300)
pygame.time.set_timer(dictado_reset,1200)
...
while True:
	for event in pygame.event.get():
		...
		if event.type == dictado_timer:
			f = open ('texto_dictado.txt','r')
			texto_dictado = f.read()
			print(texto_dictado)

		if game_active:
			if event.type == obstacle_timer:
				obstacle_group.add(Obstacle(choice(['nube','nube','nube2','nube2'])))

			if event.type == dictado_reset:
				f = open ('texto_dictado.txt','w')
				f.write('RAS, Ana y Sonia ayuda!!')
				f.close()


				#with open('texto_dictado.txt') as f:
				#	texto_dictado = f.readlines()[0]
				#	print(texto_dictado)	
		else:
